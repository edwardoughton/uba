...
def plot_sub_national_gross_cost(data, regions):
    ...
    labels = ['<10m USD', '10-20m USD', '20-30m USD','30-40m USD','40-50m USD','50-60m USD',
        '60-70m USD','70-80m USD','80-90m USD','90-100m USD','>100m USD']
    regions['bin'] = pd.cut(
        regions[metric],
        bins=bins,
        labels=labels
    ...
    # plt.subplots_adjust(top=1.5)
    fig.suptitle('Total Cost for 4G Universal Broadband using Wireless Backhaul (n={})'.format(n))
...
def plot_national_costs(national_costs):
    ...
    plot.fig.suptitle('Total Cost for Universal Broadband of 2 Mbps Per User')
